...
const flow = document.body?.dataset?.flow || "normal";
const isZonaSur = flow === "zona-sur";
const daySelect = document.getElementById("checkoutDay");
const defaultDayOptions = daySelect ? daySelect.innerHTML : "";
const zonaSurDayOptions = ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"];
...
function setDeliveryOptions() {
  ...
  if (isZonaSur) {
    daySelect.innerHTML =
      ...
      zonaSurDayOptions.map((day) => `<option value="${day}">${day}</option>`).join("");
  } else if (defaultDayOptions) {
    daySelect.innerHTML = defaultDayOptions;
  }
}
...
function toggleAddressField() {
  ...
  addressField.style.display = isZonaSur ? "block" : "none";
}
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="modal-text">
            ...
            <div class="modal-price" id="modalPrice" style="margin-top: 20px;"></div>
...
// Open modal function
function openModal(product, productId) {
  ...
  if (product.price) {
    mPrice.textContent = `$ ${product.price.toLocaleString("es-AR")}`;
...
function confirmPurchase() {
  ...
  if (!name || !phone || !day || (isZonaSur && !address)) {
    const missing = [];
    if (!name) missing.push("Nombre");
    if (!phone) missing.push("Celular");
    if (isZonaSur && !address) missing.push("Dirección");
  ...
  const whatsappMessage =
    ...
    `👤 *Cliente:* ${name}%0A` +
    `📱 *Celular:* ${phone}%0A` +
    (isZonaSur ? `📍 *Dirección:* ${address}%0A` : "") +
  ...
  const emailBody =
    ...
    `Cliente: ${name}\n` +
    `Celular: ${phone}\n` +
    (isZonaSur ? `Dirección: ${address}\n` : "") +
    `Día de Entrega: ${day}`;

  // URLs
  const whatsappNumber = isZonaSur ? "3517637431" : "3512402359";
...
function getProductTag(title) {
  if (!title) return "Selección Olivery";
  if (title.toLowerCase().includes("aceto")) return "Aceto balsámico";
  return "Aceite de oliva";
}

function renderZonaSurProducts() {
  const grid = document.getElementById("zonaSurProducts");
  if (!grid) return;

  const zonaSurOrder = [
    1, 14, 3, 4, 5, 6, 12, 9, 10, 17, 11, 13, 15, 16, 18, 7, 8,
  ];

  const fragment = document.createDocumentFragment();
  zonaSurOrder.forEach((id) => {
    const product = productsData[id];
    if (!product) return;
    const card = document.createElement("article");
    card.className = "product-card animate-on-scroll";
    card.setAttribute("data-product", String(id));
    ...
    card.innerHTML = `
      <div class="product-image">
        ...
        <div class="product-overlay"><span class="view-more">Ver detalle</span></div>
      </div>
      <div class="product-info">
        <span class="product-tag">${getProductTag(product.title)}</span>
        <h3>${product.title}</h3>
        <p class="product-brief">${brief}</p>
        <div class="product-price">$ ${product.price.toLocaleString("es-AR")}</div>
        <button class="buy-wa-btn" type="button">
...
productCards.forEach((card) => {
  card.addEventListener("click", (e) => {
    ...
    const productId = card.getAttribute("data-product");
    const product = productsData[productId];
    if (product) openModal(product, productId);
  });
});
